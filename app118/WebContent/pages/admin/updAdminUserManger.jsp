	...
	<table border="0">
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				...
				<input type="text" name="loginName" id="loginName" value="${user.loginName}"  style="width: 220px;"/><font color="red">*</font>手机号，即登录app的用户账号
			</td>
		</tr>
		<tr>
			<td  class="tdtitle">真实姓名：</td>
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="realName" id="realName" value="${user.realName}"   style="width: 220px;"/><font color="red">*</font>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="hidden" name="deviceId" id="deviceId" value="${airDevice.deviceId}">
				 <select  name="carBrand" id="carBrand"  style="width: 220px;" onchange="doCarBrandChange(this.value)">
				 	 ...
				 	 <c:forEach items="${brandList}" var="item" varStatus="index">
				 	 	<option value="${item.codeId}" <c:if test="${item.codeId eq airDevice.carBrand}">selected</c:if>>${item.codeName}</option>
				     </c:forEach>
				 </select>
				<font color="red">*</font>
			</td>
		...
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<select  name="carType" id="carType"  style="width: 220px;" onclick="">
				 	 <option value="">---请选择---</option>
		...
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="carStyle" id="carStyle" value="${airDevice.carStyle}"  style="width: 220px;"/><font color="red">*</font>
			</td>
		</tr>
			...
			<td class="tdtext">
				<select  name="carYear"  id="carYear"  style="width: 220px;">
					...
					<c:forEach items="${yearList}" var="year">
						<option value="${year}"   <c:if test="${year eq airDevice.carYear}">selected</c:if> >${year}</option>
					</c:forEach>
				</select>
				<font color="red">*</font>
			</td>
		...
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="deviceName" id="deviceName" value="${airDevice.deviceName}"  style="width: 220px;"/><font color="red">*</font>
			</td>
		</tr>
		<tr>
			<td class="tdtitle">设备Mac：</td>
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="deviceMac" id="deviceMac" value="${airDevice.deviceMac }" style="width: 220px;"/><font color="red">*</font>例如 00:17:EA:92:DC:27，不区分大小写
			</td>
		</tr>
		...
		<%-- <tr>
			...
			<td  nowrap="nowrap" class="tdtext">
				<input type="hidden" name="rechargeId" id="rechargeId" value="${deviceRecharge.rechargeId}">
				<select name="cardType" id="cardType" style="width: 220px;height: 20px;" disabled="disabled">
					<option value="">---请选择---</option>
					<c:forEach var="oneMap" items="${cardTypeMap}">
						<option value="${oneMap.key }" <c:if test="${oneMap.key eq deviceRecharge.cardType}">selected</c:if>>${oneMap.value}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="paidAmount" id="paidAmount" value="${deviceRecharge.paidAmount}" style="width: 220px;" readonly="readonly" />元
			</td>
		</tr>
		<c:if test="${hepa.hepaId!=null}">
			<tr>
				<td  class="tdtitle">过滤器方案：</td>
				<td  nowrap="nowrap" class="tdtext">
					<input type="hidden" name="hepaId" id="hepaId" value="${hepa.hepaId}">
					<select name="hepaStatus" id="hepaStatus" style="width: 220px;height: 20px;">
						<option value="">---请选择---</option>
						<c:forEach var="oneMap" items="${hepaStatusMap}">
							<option value="${oneMap.key }" <c:if test="${oneMap.key eq hepa.hepaStatus}">selected</c:if>>${oneMap.value}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
			<tr>
				<td class="tdtitle">实付金额：</td>
				<td class="tdtext" nowrap="nowrap">
					<input type="text" name="realPrice" id="realPrice" value="${hepa.realPrice}" style="width: 220px;"/>元
				</td>
			</tr>
		</c:if> --%>
		<tr align="center">
...
<script language="javascript">
	$(function() {
		initCarSeriesSelect();//初始化汽车系列，设定数据库中值为选定的下拉框
		initCarTypeSelect();//初始化汽车型号，设定数据库中值为选定的下拉框
		
		var url = "/app118/adminUserMangerAction/updUserAll";
		$("#submitBtn").click(function() {
			 $.ligerDialog.confirm('本功能仅供店长使用，修改数据存在风险，请慎重，确认修改客户信息吗？',function(yes){
				  if(yes){
...
// 							return;
// 						}
						if(carBrand==null||trim(carBrand)==''||carBrand=="---请选择---"){
							$.ligerDialog.warn("汽车品牌不能为空，请选择。");
							$("#carBrand").focus();
							return;
						}
						if(carSeries==null||trim(carSeries)==''||carSeries=="---请选择---"){
							$.ligerDialog.warn("汽车系列不能为空，请选择。");
							$("#carSeries").focus();
							return;
						}
						if(carType==null||trim(carType)==''||carType=="---请选择---"){
						...
						if(carYear==null||trim(carYear)==''||carYear=="---请选择---"){
	...
	function initCarSeriesSelect(){
		...
		carSeriesSelect.options.length=0; 
	 	...
	 	carSeriesSelect.options[carSeriesSelect.options.length] = objOption;
		<c:forEach items="${carSeriesList}" var="item" varStatus="index">
			if('${item.pCode}'=='${airDevice.carBrand}'){   
			   var objOption = new Option("${item.codeName}","${item.codeId}");   
			   carSeriesSelect.options[carSeriesSelect.options.length] = objOption;   
			 }
	     </c:forEach>
	     
	    for(var i=0;i<carSeriesSelect.options.length;i++) {  
 	           if(carSeriesSelect.options[i].value == "${airDevice.carSeries}"){  
 	            	carSeriesSelect.options[i].selected = true;  
 	               	break;  
 	            }  
	     } 
	...
	function initCarTypeSelect(){
		...
		carTypeSelect.options.length=0; 
	 	...
	 	carTypeSelect.options[carTypeSelect.options.length] = objOption;
		<c:forEach items="${carTypeList}" var="item" varStatus="index">
			if('${item.pCode}'=='${airDevice.carSeries}'){   
			   var objOption = new Option("${item.codeName}","${item.codeId}");   
			   carTypeSelect.options[carTypeSelect.options.length] = objOption;   
			 }
	     </c:forEach>
	    ...
	    for(var i=0;i<carTypeSelect.options.length;i++) {  
 	           if(carTypeSelect.options[i].value == "${airDevice.carType}"){  
 	            	carTypeSelect.options[i].selected = true;  
 	               	break;  
 	            }  
	     } 
