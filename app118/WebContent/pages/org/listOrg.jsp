...
<body>
	...
	<div id="toptoolbar" class="l-toolbar" ligeruiid="toptoolbar" style="width:99%;">
		<div class="l-toolbar-item l-panel-btn l-toolbar-item-hasicon"
			toolbarid="item-1" jQuery1405912963467="22" onclick="addOrg();">
			<SPAN>增加组织机构</SPAN>
		...
		<div class="l-toolbar-item l-panel-btn" toolbarid="item-2"
			jQuery1405912963467="23" onclick="updOrg();">
			<SPAN>&nbsp;编辑组织机构</SPAN>
			<div class=l-panel-btn-l></div>
			<div class=l-panel-btn-r></div>
		...
		<div class="l-toolbar-item l-panel-btn" toolbarid="item-3"
			jQuery1405912963467="24" onclick="delOrg();">
			<SPAN>&nbsp;删除组织机构</SPAN>
			<div class=l-panel-btn-l></div>
			<div class=l-panel-btn-r></div>
...
<script type="text/javascript">
	var dialog=null;//弹出对话框
     var url = "/app118/orgAction/listOrgByPager";
     $(function (){
     	init(url);
     ...
     function forClean(){
   	 ...
   	 //增加组织机构
   	 function addOrg(){
   		var url ="/app118/orgAction/toAddOrg";
		 ...
		 var len=rowid.length;
			 if(len>1){
			    ...
			    if(rowid[0].orgPid==0){//根结点不可以修改
					   $.ligerDialog.warn("【"+rowid[0].orgName+"】，组织机构不能修改。");
					   return; 
				} 
				id+=rowid[0].orgId;
				params="?id="+id;
   		...
   		var url ="/app118/orgAction/toUpdOrg"+params;
  		dialog=$.ligerDialog.open({ 
  				  url:url, 
  				  height:520,
  				  isResize:true,
		...
		if(rowid.length==0){
		 ...
		 for(var j=0;j<len;j++){
		   if(rowid[j].orgPid==0){//根结点不可以删除
			   $.ligerDialog.warn("【"+rowid[j].orgName+"】，组织机构不能删除。");
			   return; 
		   }
		   if(j!=len-1){
			   ids+=rowid[j].orgId+",";			
		 ...
		 var url="/app118/orgAction/delOrg";
		  $.ligerDialog.confirm('确认删除选择的组织机构吗？',function(yes){
			  if(yes){
		      		$.ajax({
			      			type:'POST',
							url:url,
  		...
  		*/
  		window['g']= $("#maingrid").ligerGrid({
            	  ...
            	  height:'99%',
  	              ...
  	              headerRowHeight:28,
  	              columns: [
		  	          { display: '组织机构名称', name: 'orgName', width: '15%'},
	  	              { display: '组织机构编号', name: 'orgNo', width: '10%'},
