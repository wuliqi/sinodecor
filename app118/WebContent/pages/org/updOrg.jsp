	...
	<table border="0" align="center">
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				...
				<input type="text" name="orgName"  id="orgName" value="${org.orgName}" style="width: 252px;height: 20px;"/>
				<font color="red">*</font>
			</td>
		</tr>
		<tr>
			...
			<td  nowrap="nowrap">
				<input type="text" name="orgNo" id="orgNo" value="${org.orgNo}" style="width: 252px;height: 20px;">
				<font color="red">*</font>
			</td>
		</tr>
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<select name="orgPid" id="orgPid"  style="width: 252px;height: 20px;">
					...
					<c:forEach var="oneMap" items="${orgMap}">
						<option value="${oneMap.key }" <c:if test="${oneMap.key eq org.orgPid}">selected</c:if> >${oneMap.value}</option>
					</c:forEach>
				</select>
				<font color="red">*</font>
			</td>
			...
			<td class="tdtext" nowrap="nowrap">
				<select name="orgType" id="orgType"  style="width: 252px;height: 20px;">
					...
					<c:forEach var="oneMap" items="${codeList}">
						<option value="${oneMap.codeId }" <c:if test="${oneMap.codeId eq org.orgType}">selected</c:if> >${oneMap.codeName}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="mobile"  id="mobile" value="${org.mobile}" style="width: 252px;height: 20px;"/>
			</td>
		</tr>
		<tr>
			<td  class="tdtitle">地理位置：</td>
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="abbr"  id="abbr" value="${org.abbr}" style="width: 252px;height: 20px;"/>
		...
		<tr>
			...
			<td nowrap="nowrap">
				<textarea rows="8" cols="30" name="remark" id="remark">${org.remark}</textarea>
			</td>
		</tr>
		<tr align="center">
			<td colspan="2" > 
...
<script language="javascript">
	$(function() {
		var url = "/app118/orgAction/updOrg";
	...
	function tips(){
		...
		if(!(msg=='null'||msg=='')){
			if(msg == "success"){
				$.ligerDialog.success("修改组织机构成功。",function(){
					parent.closeDialog();	
				});
			}else{
				$.ligerDialog.error("修改组织机构失败。",function(){
					parent.closeDialog();	
				});
			}
		}
