...
<body>
	...
	<div id="toptoolbar" class="l-toolbar" ligeruiid="toptoolbar" style="width:99%;">
		<div class="l-toolbar-item l-panel-btn l-toolbar-item-hasicon"
			toolbarid="item-1" jQuery1405912963467="22" onclick="addUser();">
			<SPAN>添加用户</SPAN>
		...
		<div class="l-toolbar-item l-panel-btn" toolbarid="item-3"
			jQuery1405912963467="24" onclick="delUser();">
			<SPAN>&nbsp;删除用户</SPAN>
			<div class=l-panel-btn-l></div>
			<div class=l-panel-btn-r></div>
...
<script type="text/javascript">
	var dialog=null;//弹出对话框
     ...
     function forClean(){
   	 ...
   	 //增加
   	 function addUser(){
   		var url ="/app118/userAction/toInsertUser";
		...
		if(rowid.length==0){
		 ...
		 var len=rowid.length;
			 if(len>1){
				...
				var sex=rowid[0].sex;
				params="?loginName="+loginName+"&realName="+encodeURI(realName)+"&sex="+encodeURI(sex);

			 }
		}
   		var url ="/app118/userAction/toUpdUser"+params;
  		dialog=$.ligerDialog.open({ 
  				  url:url, 
  				  height:520,
  				  isResize:true,
  		...
  		*/
  		window['g']= $("#maingrid").ligerGrid({
            	  ...
            	  height:'99%',
  	              ...
  	              columns: [
		  	          { display: '用户标识', name: 'userId', width: '10%'},
		  	          { display: '所属组织机构', name: 'orgName', width: '10%'},
	  	              { display: '用户名', name: 'loginName', width: '20%'},
	  	              { display: '姓名', name: 'realName', width: '20%' },
	  	              { display: '性别', name: 'sex', width: '15%' },
