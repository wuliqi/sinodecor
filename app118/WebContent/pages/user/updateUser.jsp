	...
	<table border="0">
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="loginName" id="loginName" value="${loginName}"  style="width: 220px;" readonly="readonly"/><font color="red">*</font>手机号，即登录app的用户账号
			</td>
		</tr>
		<tr>
			<td  class="tdtitle">真实姓名：</td>
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="realName" id="realName" value="${realName}"   style="width: 220px;" readonly="readonly"/><font color="red">*</font>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td  nowrap="nowrap" class="tdtext">
				<%--  <select  name="saleUserId" id="saleUserId"  style="width: 220px;">
				 	 <option value="">---请选择---</option>
				 	 <c:forEach items="${saleUserList}" var="item" varStatus="index">
				 	 	<option value="${item.userId}" <c:if test="${item.userId eq userId}">selected</c:if>>${item.realName}</option>
				     </c:forEach>
				 </select> --%>
...
<script language="javascript">
	$(function() {
		var url = "/app118/userAction/addUserAll?op=upd";
