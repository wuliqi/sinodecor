	...
	<table border="0">
		<tr>
			...
			<td class="tdtext" nowrap="nowrap">
				<input type="text" name="loginName" id="loginName"  style="width: 220px;"/><font color="red">*</font>手机号，即登录后台用户账号
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<textarea rows="15" cols="26"  name="roleNames" id="roleNames" readonly="readonly"></textarea>
...
<div id="target1" style="height:300px;position:relative;display:none;overflow:auto;">
	<div>
	     <c:forEach items="${list}" var="role" varStatus="index">
	     	<input type="checkbox" name="role" id="role" value="${role.roleId }">${role.roleName}<br/>
	     </c:forEach>
   	</div>
</div>
...
<script language="javascript">
	$(function() {
		var url = "/app118/accountAction/addAccount";
	...
	function tips(){
		...
		if(!(msg=='null'||msg=='')){
			if(msg == "success"){
				$.ligerDialog.alert("添加用户成功。",function(){
					parent.closeDialog();	
				});
			}else if(msg=="exist"){
				...
				$.ligerDialog.alert("添加用户失败。",function(){
					parent.closeDialog();	
				});
			}
		}
