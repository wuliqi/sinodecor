...
<body>
	...
	<div id="toptoolbar" class="l-toolbar" ligeruiid="toptoolbar" style="width:99%;">
		<div class="l-toolbar-item l-panel-btn l-toolbar-item-hasicon"
			toolbarid="item-1" jQuery1405912963467="22" onclick="addAccount();">
			<SPAN>添加用户</SPAN>
		...
		<div class="l-toolbar-item l-panel-btn" toolbarid="item-2"
			jQuery1405912963467="23" onclick="updAccount();">
			<SPAN>&nbsp;修改用户</SPAN>
			<div class=l-panel-btn-l></div>
			<div class=l-panel-btn-r></div>
		...
		<div class="l-toolbar-item l-panel-btn" toolbarid="item-3"
			jQuery1405912963467="24" onclick="delAccount();">
			<SPAN>&nbsp;删除用户</SPAN>
			<div class=l-panel-btn-l></div>
			<div class=l-panel-btn-r></div>
...
<script type="text/javascript">
	var dialog=null;//弹出对话框
     var url = "/app118/accountAction/listAccount";
     $(function (){
     ...
     function forClean(){
   	 ...
   	 //增加用户
   	 function addAccount(){
   		var url ="/app118/accountAction/toInsertAccount";
		...
		if(rowid.length==0){
			 ...
			 if(len>1){
				...
				var roleIds=rowid[0].roleIds;
				var roleNames=rowid[0].roleNames;
				params="?loginName="+loginName+"&realName="+encodeURI(realName)+"&sex="
					+encodeURI(sex)+"&userId="+id+"&orgId="+orgId+"&roleIds="+roleIds+"&roleNames="+encodeURI(roleNames);
	
			 }
		}
   		var url ="/app118/accountAction/toUpdAccount"+params;
  		dialog=$.ligerDialog.open({ 
  				  url:url, 
  				  height:520,
  				  isResize:true,
		...
		if(rowid.length==0){
		 ...
		 var url="/app118/accountAction/delAccount";
		  $.ligerDialog.confirm('确认删除选择的后台用户吗？',function(yes){
			  if(yes){
		      		$.ajax({
			      			type:'POST',
							url:url,
     ...
     function updUserType(){
  		...
  		if(rowid.length==0){
  		 ...
  		 var len=rowid.length;
  			 if(len>1){
  				...
  				if(userType!="1"){
  					$.ligerDialog.warn("【"+rowid[0].realName+"】为app用户，无需转换。");
  					return;
  				}
  			 }
  		}
    		var url ="/app118/userAction/updUserType";
    	    $.ligerDialog.confirm('确认将选择的用户转为app用户吗？',function(yes){
  		  if(yes){
  	      		$.ajax({
  		      			type:'POST',
  						url:url,
  						data:{userId:id,userType:"3"},
  						dataType:'json',
  		      			success:function(msg){
  		      				if(msg.flag=1){
  		      					$.ligerDialog.success("转app用户成功。");
  		      					g.loadData();
  		      				}else{
  		      					$.ligerDialog.error("转app用户失败。");
  		      				}
  		      			},
  		      			error:function(){
  		      				$.ligerDialog.error("操作失败。");
   	 ...
   	 //查看明细
   	 function viewAccount(userId,orgName){
   		var url ="/app118/accountAction/viewAccount?userId="+userId+"&orgName="+encodeURI(orgName);
  		dialog=$.ligerDialog.open({ 
  				  url:url, 
  				  height:520,
  				  isResize:true,
  		...
  		window['g']= $("#maingrid").ligerGrid({
            	  ...
            	  height:'99%',
  	              ...
  	              columns: [
	  	              ...
	  	              { display: '性别', name: 'sex'},
	  	              { display: '所属角色', name: 'roleNames'},
	  	              { display: '用户类型', name: 'userTypeCn'},
	  	              { display: '创建时间', name: 'createTime'},
	  	              { display: '查看', name: '查看', width: '5%',render: function (row){
			             	var parms="\""+row.userId+"\""+","+"\""+row.orgName+"\"";
			             	var html="<a href='javascript:viewAccount("+parms+");'>查看</a>";
